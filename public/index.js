...
function getProducts(page){
    axios.get(`http://54.199.192.67:4000/products/?page=${page}`).then((products)=>{
...
document.addEventListener('click',(e)=>{
    ...
    if (e.target.className=='shop-item-button'){
        ...
        //console.log(prodId);
        axios.post('http://54.199.192.67:4000/cart', { productId: prodId}).then(data => {
            if(data.data.error){
    ...
    if (e.target.className=='purchase-btn'){
        ...
        axios.post(`http://54.199.192.67:4000/create-order`).then(response=>{
            getCartItems();
...
function getCartItems(){
    axios.get('http://54.199.192.67:4000/cart').then(carProducts => {
...
function deleteCartItem(e, prodId){
    e.preventDefault();
    axios.post('http://54.199.192.67:4000/cart-delete-item', {productId: prodId})
